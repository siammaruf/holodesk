...
import { Tool, TileMode, SpecialTile, Layer } from '~/types/pixi'
...
const LeftBar:React.FC<LeftBarProps> = ({ tool, tileMode, selectTool, selectTileMode, specialTile, selectEraserLayer, eraserLayer }) => {
    ...
    return (
        <div className='w-[56px] bg-[#1a1a2e]/60 backdrop-blur-sm flex flex-col items-center py-3 gap-2 border-r border-white/5 shrink-0 overflow-y-auto no-scrollbar'>
            <ToolButton selected={tool === 'Hand'} label={'Hand'} onClick={() => selectTool('Hand')}>
                <HandRaisedIcon className='h-5 w-5 text-white/90'/>
            </ToolButton>
            <ToolButton selected={tool === 'ZoomIn'} label={'Zoom In'} onClick={() => selectTool('ZoomIn')}>
                <MagnifyingGlassPlusIcon className='h-5 w-5 text-white/90'/>
            </ToolButton>
            <ToolButton selected={tool === 'ZoomOut'} label={'Zoom Out'} onClick={() => selectTool('ZoomOut')}>
                <MagnifyingGlassMinusIcon className='h-5 w-5 text-white/90'/>
            </ToolButton>
            <ToolButton selected={tool === 'Eraser'} label={'Eraser'} onClick={() => selectTool('Eraser')}>
                <Eraser className='h-5 w-5 text-white/90'/>
            </ToolButton>

            <div className='w-8 h-px bg-white/10 my-1'/>

            <ToolButton selected={tileMode === 'Single'} label={'Single Tile'} onClick={() => selectTileMode('Single')}>
                <Square className='h-5 w-5 text-white/90'/>
            </ToolButton>
            <ToolButton selected={tileMode === 'Rectangle'} label={'Rectangle'} onClick={() => selectTileMode('Rectangle')}>
                <GridFour className='h-5 w-5 text-white/90'/>
            </ToolButton>

            <div className='w-8 h-px bg-white/10 my-1'/>

            <ToolButton selected={false} onClick={toggleShowGizmos} label={'Toggle Special Tiles'} className={specialTile !== 'None' ? 'pointer-events-none text-white/30' : ''}>
                {showGizmos ? <EyeSlash className='h-5 w-5 text-white/90'/> : <Eye className='h-5 w-5 text-white/90'/>}
            </ToolButton>

            <div className='w-8 h-px bg-white/10 my-1'/>

            <ToolButton selected={false} label={'Undo'} onClick={undo} disabled={!undoEnabled}>
                <ArrowUUpLeft className='h-5 w-5 text-white/90'/>
            </ToolButton>
            <ToolButton selected={false} label={'Redo'} onClick={redo} disabled={!redoEnabled}>
                <ArrowUUpRight className='h-5 w-5 text-white/90'/>
            </ToolButton>
